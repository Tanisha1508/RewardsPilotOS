...
| ~~A11~~ | TEST | ~~**Improve ranking for card-agnostic questions**~~ — **done 2026-07-31** | Diagnosed rather than assumed: "which card should I use to book a hotel" returned six chunks **all from one card**, four from one document, top hit about income eligibility — and scores clustered at 0.0293/0.0292/0.0284, so ordering was near-arbitrary and volume won. `_diversify` now takes each document's best chunks up to `MAX_PER_DOC=2`, then backfills. Evidence breadth on comparison questions: **2.00 → 2.60 issuers**, **3.20 → 4.00 documents** per query. Backfill guarantees it never returns fewer results. **The doc-level eval was blind to this** — it dedupes by document at k=10, so chunk-level crowding was invisible; measured directly instead |
