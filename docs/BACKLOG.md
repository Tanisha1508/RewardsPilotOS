...
| A10 | FN | **Wire retrieval metadata filters** (`agents/knowledge/behavior.py`) | Implemented, BUILD_SPEC §6 step 3, never called — retrieval runs unfiltered. Real upside: issuer filtering would have prevented the Redeem cross-issuer bug fixed 2026-07-30. **Fix its stale issuer map in the same change** — it still maps "voyager"/"trailblazer" to fixture issuers removed from the corpus by KL 35. The two sibling modules (`agents/memory/behavior.py`, `agents/graph/behavior.py`) are also unwired; see `docs/WIRING_SWEEP.md` round 2 |
